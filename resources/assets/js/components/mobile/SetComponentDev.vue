<template>
    ...
    <div class="products">
      <div class="item" v-for="product in set.products" :data-productid="product.code">
        ...
        <div class="descr">
         <a :href="'/' + $lang + '/' + site + '/catalog/' + product.category.alias + '/' + product.alias" class="nameProduct" @click="viewProductGA(product)">
             {{ product.translation.name }}
         </a>
          <div class="price">
              <span>{{ product.personal_price.price }}  <span v-if="product.discount == 0">{{ $currency }}</span> </span>
              <span v-if="product.discount > 0">{{ product.personal_price.old_price }} {{ $currency }}</span>
              <span v-else></span>
          </div>

          <add-to-cart-set-product-mob :set="set" :product="product" :site="site"></add-to-cart-set-product-mob>

        </div>
      </div>

      <div class="additional-block">
          <p><a href="#">{{ trans.vars.DetailsProductSet.weShip }}</a> {{ trans.vars.DetailsProductSet.shipRomaniaEU }}</p>
          <p><a href="#">{{ trans.vars.DetailsProductSet.canReturn }}</a> {{ trans.vars.Contacts.returnDays }}</p>
          <p>{{ trans.vars.DetailsProductSet.moreDetailsShipping }} <a href="#">{{ trans.vars.DetailsProductSet.here }}</a> </p>
          <p>{{ trans.vars.DetailsProductSet.gotQuestion }}</p>
      ...
      <!-- <div class="cartButton" @click="openSizesDropDown">
        <span> View available sizes </span>
      </div>
      <div class="cartButton" v-if="addTocartBtn">
        <span> add to
...
export default {
    ...
    methods: {
        ...
        viewProductGA(product){
            bus.$emit('ga-event-clickImpresions', {product: product, list: "One col"});
        },
...
<style media="screen">
    ...
    .additional-block  .cartButton{
        background-image: url(/fronts/img/backgrounds/buttons.jpg);
        background-size: 100% 100%;
        height: 55px;
        cursor: pointer;
        font-family: "GillSans-Light", Georgia, "Times New Roman", Times, serif;
        font-size: 18px;
        color: #B22D00;
