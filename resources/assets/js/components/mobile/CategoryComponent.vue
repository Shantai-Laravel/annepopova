        ...
        <div class="oneCateProd">
          <div class="oneProduct">
            ...
            <div class="price">
                ...
                <span v-if="product.discount > 0">{{ product.personal_price.old_price }} </span>
                <span v-if="product.discount > 0" class="currency">{{ $currency }}</span>
            </div>
          </div>
        </div>
      </div>
...
export default {
    ...
    methods: {
        viewProductGA(product){
            ...
            bus.$emit('ga-event-viewProduct', {product: product, actionField: category.translation.name});
        },
        parseProducts(){
            let productArr = [];
            let products = [];
