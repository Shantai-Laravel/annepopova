        ...
        <div class="orderSummary">
            <div>
                ...
                <div class="bloc last">
                    ...
                    <span>{{ parseFloat(amount)  }} {{ $mainCurrency }}</span>
                </div>
                <!-- <span v-if="mode != 'order-shipping' && mode != 'order-payment'">
                    <span v-if="notInStoc">
                        <a data-toggle="modal" data-target="#checkStocks" href="#" @click="setGuest"><button class="onSubmit">{{ trans.vars.Orders.order }}</button></a>
                    </span>
                    <span v-else>
                ...
                axios.post('/' + this.$lang + '/' + this.site +'/set-country-delivery', {
                    ...
                    .then(response => {
                        if (!this.initiateCheckout) {
                            ...
                            bus.$emit('ga-event-initiateCheckout', {promo: false, products: {prods: this.cartProds, subprods: this.cartSubprods}, amount: this.amount});
                        }
                    })
                    .catch(e => {
                        this.serverError = 'A avut loc o problema tehnica!';
