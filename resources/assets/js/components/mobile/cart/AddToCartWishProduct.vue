<template>
    <div class="">
        <!-- <div class="sizeContainer">
          <div>{{ trans.vars.DetailsProductSet.size }}</div>
          <div class="chooseSize animated">

            <label for="colSize" class="collSize" v-for="subproduct in product.subproducts">
              <input type="radio" name="colSize3" @click="setProduct(product, subproduct)"/>
              <span class="check">{{ subproduct.parameter_value.translation.name }}</span>
            </label>
        ...
        <div class="buttonsSet">
          <div class="wishButton" @click="removeProductWish(product.id)">
            <span>
              remove
            </span>
          </div>
          <div class="cartButton" @click="moveToCart(product)">
...
export default {
    ...
    methods: {
        ...
        addProductToCartAction(product) {
            axios.post('/' + this.$lang + '/' + this.site + '/add-product-to-cart-from-wish', {
